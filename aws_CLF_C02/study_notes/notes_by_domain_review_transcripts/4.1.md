...
START
  |
  v
Have you rightsized the workload and enabled elasticity?
  |
  +-- No --> Rightsize resources and configure Auto Scaling first.
  |
  +-- Yes
        |
        v
Is the workload required to run on dedicated physical hardware
or use single-tenant infrastructure?
        |
        +-- Yes --> Do you need control and visibility over the
        |           physical server?
        |             |
        |             +-- Yes --> Dedicated Host
        ...
        +-- No
              |
              v
Do you need guaranteed EC2 capacity in a specific
Availability Zone?
              |
              +-- Yes --> Capacity Reservation
              |           - Reserves capacity in a specific AZ
              |           - Useful when capacity must be available
              |           - Does not automatically provide a discount
              |           - Charges apply while reserved
              |
              +-- No
                    |
                    v
Can the workload tolerate interruption or termination?
                    |
                    +-- Yes
                    |     |
                    |     v
                    |   Is the workload flexible and able to use
                    |   spare capacity?
                    |     |
                    |     +-- Yes --> Spot Instances
                    |     |           - Lowest EC2 price
                    ...
                    +-- No
                          |
                          v
Is usage expected to be steady and predictable?
                          |
                          +-- No --> On-Demand Instances
                          ...
                          +-- Yes
                                |
                                v
Do you want flexibility across compute services,
instance families, operating systems, or regions?
                                |
                                +-- Yes --> Savings Plans
                                |           - Commit to a consistent amount
                                |             of compute usage per hour
                                |           - Lower prices than On-Demand
                                |           - More flexible than Standard RIs
                                |           - Applies to eligible compute
                                |             usage such as EC2, Lambda,
                                |             and Fargate
                                |
                                +-- No
                                      |
                                      v
Do you need a specific EC2 configuration and can commit
for 1 or 3 years?
                                      |
...
Production baseline
  ...
  |      Predictable, always-running workload
  |
  +-- On-Demand Instances
  |      Unplanned or temporary additional capacity
  |
  +-- Spot Instances
         Flexible batch jobs, testing, analytics, or
...
Need the lowest possible price?
  ...
  |
  +-- Cannot tolerate interruptions --> Continue evaluating commitment

Need no commitment and maximum flexibility?
  |
  +-- Yes --> On-Demand Instances

Have predictable EC2 usage for 1 or 3 years?
  |
  +-- Need maximum discount and stable configuration?
  |      --> Standard Reserved Instance
  |
  +-- Need to change configuration over time?
  |      --> Convertible Reserved Instance
  |
  +-- Need flexible coverage across compute services?
         --> Savings Plan

Need dedicated physical hardware?
  |
  +-- Need host-level control or licensing support?
  |      --> Dedicated Host
  |
  +-- Need single-tenancy only?
         --> Dedicated Instance

Need guaranteed EC2 capacity in a particular AZ?
  |
  +-- Yes --> Capacity Reservation
```
